...
pub fn McpServersTab(
  ...
  let join_args = |args: &[String]| -> String { args.join(" ") };
    ...
    div {
      ...
      Modal {
        ...
        ModalFooter {
          ...
          PrimaryButton {
            onclick: move |_| {
                let args: Vec<String> = server_form_args()
                    .lines()
                    .map(|s| s.to_string())
                    .filter(|s| !s.is_empty())
                    .collect();
                let new_server = McpServerConfig {
                    name: if server_form_name().is_empty() {
                ...
                if let Ok(mut config) = AppConfig::load() {
                    if server_is_adding() {
                        ...
                        .servers
                        .iter_mut()
                        .find(|s| s.name == editing_server().unwrap_or_default())
                    {
                        *s = new_server;
                    }
                    let _ = config.save();
