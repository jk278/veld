...
impl AiClient {
  ...
  fn get_active_provider_config() -> Result<(String, String, String, String)> {
    ...
    let active_id = config.ai.active_provider.ok_or(AiError::NoActiveProvider)?;
    ...
    let base_url = provider
      .base_url
      .ok_or_else(|| AiError::Api(format!("Base URL not configured for {}", active_id)))?;
  ...
  fn parse_anthropic_response(body: &str) -> Result<String> {
    ...
    let resp: ClaudeResponse = serde_json::from_str(body).map_err(|e: serde_json::Error| {
